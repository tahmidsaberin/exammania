...
function groupHistory(history: Attempt[], divisions: Division[]): DivisionGroup[] {
  ...
  for (const [divKey, subjectMap] of divGroups.entries()) {
    ...
    if (divKey === "__common__") {
      divisionName = "Common Subjects";
    } else {
      const div = divMap.get(divKey);
      if (div) {
...
function pctOf(a: Attempt) {
  return a.totalScore > 0 ? Math.round((a.score / a.totalScore) * 100) : 0;
}
...
function SubjectAccordion({ subject }: { subject: SubjectGroup }) {
  ...
  const avgPct = total > 0
    ? subject.attempts.reduce((s, a) => s + pctOf(a), 0) / total
    : 0;
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <div className="hidden sm:flex items-center gap-1.5">
            {subject.attempts.slice(0, 3).map((a) => {
              const p = pctOf(a);
              return (
                <span key={a.id} className={clsx("inline-flex rounded-full px-2 py-0.5 text-xs font-semibold", scoreBadge(p))}>
                  {p}%
                </span>
              );
            })}
            {total > 3 && (
              <span className="text-xs text-gray-400 dark:text-gray-500">+{total - 3}</span>
            )}
          </div>
          <ChevronDownIcon
            className={clsx("h-5 w-5 flex-shrink-0 text-gray-400 transition-transform duration-200", open && "rotate-180")}
          />
        </div>
      </button>
